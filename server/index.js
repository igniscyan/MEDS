...
app.post("/insert/patient_encounter/:patientId", (req, res) => {
  ...
  const patientEncountersTableFields = [
    "patient_id",
    "gyn",
    "pregnant",
  ...
  const patiendtId = req.params.patientId;
  const valuesArray = Object.values(req.body); //array of values from request
  console.log(valuesArray);
  valuesArray.unshift(patientId);
  const insertPatientQuery = `INSERT INTO patient_encounters (${patientEncountersTableFields.join(
    ","
  )}) VALUES (${patientEncountersTableFields.forEach((field, idx) => "?,")});`;
});
